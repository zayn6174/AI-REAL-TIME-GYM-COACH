...
from detectors.leg_press import LegPressDetector
from detectors.leg_raises import LegRaisesDetector
from detectors.mountain_climbers import MountainClimbersDetector
from detectors.reverse_crunches import ReverseCrunchesDetector
...
class VideoProcessorClass(VideoProcessorBase):
    def __init__(self):
        ...
        self._detectors = {
            ...
            "Dumbbell Bicep Curls": BicepsCurlDetector(),
            "Shoulder Press": ShoulderPressDetector(),
            "Lunges": LungesDetector(),
            "Crunches": CrunchesDetector(),
            "Leg Press": LegPressDetector(),
            "Leg Raises": LegRaisesDetector(),
            "Mountain Climbers": MountainClimbersDetector(),
            "Reverse Crunches": ReverseCrunchesDetector(),
            "Plank": PlankDetector()
        }
    ...
    def _draw_overlays(self, img, metrics, ex_type):
        if ex_type == "Squats":
        ...
        elif ex_type == "Dumbbell Bicep Curls":
            ...
            self._draw_crunches_overlays(img, metrics)

        elif ex_type == "Leg Press":
            self._draw_leg_press_overlays(img, metrics)
        ...
        elif ex_type == "Reverse Crunches":
            self._draw_reverse_crunches_overlays(img, metrics)

        elif ex_type == "Mountain Climbers":
            self._draw_mountain_climbers_overlays(img, metrics)
    ...
    def _draw_crunches_overlays(self, img, metrics):
        h, _ = img.shape[:2]

        cv2.putText(
            img,
            f"CRUNCH: {metrics['crunch_status']}",
            (20, h - 20),
            cv2.FONT_HERSHEY_SIMPLEX,
            1,
            (0, 255, 0),
    ...
    def _draw_leg_press_overlays(self, img, metrics):
        h, _ = img.shape[:2]
    ...
    def _draw_leg_raise_overlays(self, img, metrics):
        ...
        cv2.putText(
            img,
            f"LEGS: {metrics['movement_status']} | "
            f"FORM: {metrics['leg_status']}",
            (20, h - 20),
            cv2.FONT_HERSHEY_SIMPLEX,
            1,
            (0, 255, 0),
    ...
    def _draw_plank_overlays(self, img, metrics):
        ...
        cv2.putText(
            img,
            f"FORM: {metrics['form_status']} | "
            f"ANGLE: {metrics['body_angle']}",
            (20, h - 20),
            cv2.FONT_HERSHEY_SIMPLEX,
            1,
            (0, 255, 0),
    ...
    def _draw_reverse_crunches_overlays(self, img, metrics):
        h, _ = img.shape[:2]

        cv2.putText(
            img,
            f"MOVEMENT: {metrics['movement_status']} | "
            f"LEGS: {metrics['leg_status']}",
            (20, h - 20),
            cv2.FONT_HERSHEY_SIMPLEX,
            1,
            (0, 255, 0),
    ...
    def _draw_mountain_climbers_overlays(self, img, metrics):
        h, _ = img.shape[:2]

        cv2.putText(
            img,
            f"LEFT: {metrics['left_knee_status']} | "
            f"RIGHT: {metrics['right_knee_status']}",
            (20, h - 20),
            cv2.FONT_HERSHEY_SIMPLEX,
            1,
            (0, 255, 0),
    ...
    def recv(self, frame):
        ...
        # OpenCV frame is BGR.
        # MediaPipe Tasks expects SRGB/RGB.
